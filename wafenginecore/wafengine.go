...
func (waf *WafEngine) StartProxyServer(innruntime innerbean.ServerRunTime) {
	...
	go func(innruntime innerbean.ServerRunTime) {
		...
		} else if (innruntime.ServerType) == "http" {
			defer func() {
				e := recover()
				if e != nil { // 捕获该协程的panic 111111
		...
		} else if (innruntime.ServerType) == "tcp" {
			fmt.Println("Start TCP1:" + strconv.Itoa(innruntime.Port)) //映射端口

			var webHost model.Hosts
			global.GWAF_LOCAL_DB.Where("Port", strconv.Itoa(innruntime.Port)).Find(&webHost)

			targetAddr := webHost.Remote_ip + ":" + strconv.Itoa(webHost.Remote_port)
			RemoteAddr := webHost.Remote_ip
			RemotePort := strconv.Itoa(webHost.Remote_port)
			fmt.Println("访问目的地2：" + targetAddr)

			//defer releaseResources(stats) // 在函数返回时释放资源

			proxy := NewTCPProxy(":"+strconv.Itoa(innruntime.Port), RemoteAddr, RemotePort)
...
func NewTCPProxy(listenAddr, RemoteAddr string, RemotePort string) *ConnectionStats {
	return &ConnectionStats{
		ConnectionStats: conf.ConnectionStats{
			LocalPort:  listenAddr,
...
func (stats *ConnectionStats) Serve() error {
	defer releaseResources(stats) // 在函数返回时释放资源（运行前释放资源）
		...
		case <-ticker.C:
			...
			if cs.TotalBytes > cs.TotalBytesOld {
				...
				if cs.TotalBytes >= gb {
					...
					//sql.UpdateForwardGb(cs.Id, cs.TotalGigabyte)	//判断是什么
					cs.TotalBytes = cs.TotalBytes - gb
				}
				cs.TotalBytesOld = cs.TotalBytes
				//sql.UpdateForwardBytes(cs.Id, cs.TotalBytes)	//判断是什么
